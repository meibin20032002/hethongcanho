...
* @subpackage	Classes
*/
class BdsModelCategory extends BdsClassModelItem
{
	/**
	* View list alias
	*
	...
	*/
	public function getLayout()
	{
		...
		return $jinput->get('layout', 'category', 'STRING');
	}
	...
	*/
	...
	{
		$conditions = array('`sub_category` = '.(int) $table->sub_category);
		return $conditions;
	}
	...
	*/
	public function getTable($type = 'category', $prefix = 'BdsTable', $config = array())
	{
	...
	*/
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_bds.edit.category.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('category.id') == 0)
			{
				...
				$data->title = null;
				$data->alias = null;
				$data->sub_category = $jinput->get('filter_sub_category', $this->getState('filter.sub_category'), 'INT');
				$data->image = null;
	...
	*/
	...
	{
		//FROM : Main table
		$query->from('#__bds_categories AS a');
