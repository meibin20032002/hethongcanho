...
* @subpackage	
*/
class BdsClassControllerItem extends JControllerForm
{
	/**
	...
	*/
	public function add()
	{
		...
		$this->applyRedirection($result, array(
			'com_bds.' . $this->view_list . '.default',
			'com_bds.' . $this->view_item . '.' . $this->getLayout('edit')
		));
	}
	...
	*/
	public function delete()
	{
		...
		else
		{
			...
			{
				$result = true;
				$this->setMessage(JText::plural('BDS_ITEMS_SUCCESSFULLY_DELETED', count($cid)));
			}
			else
			{
	...
	*/
	...
	/**
	* Method to upload and update a file value.
	*
	* @access	protected
	* @param	JModel	$model	The context model.
	* @param	string	$fieldName	The file field name to treat.
	* @param	array	$options	Uploader options.
	*
	* @return	void
	*/
	protected function updateFileField($model, $fieldName, $options = array())
	{
		// Automatically build the field directory.
		$uploadDirectory = '[DIR_' . strtoupper($model->getNameList() . '_' . $fieldName) . ']';

		try
		{
			$changedValue = BdsHelperFile::updateFile($model, $fieldName, $options, $uploadDirectory);
		}
		catch(Exception $e)
		{
