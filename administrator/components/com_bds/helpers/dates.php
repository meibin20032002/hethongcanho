...
class BdsHelperDates
{
	...
	static function getSqlDate($strtime, $formats, $toSqlString = false)
	{
		...
		foreach($formats as $format)
		{
			...
			if (preg_match($regex, $strtime))
			{
				...
				$mktime = self::timeFromFormat($strtime, $format);
				if (!$mktime)
					return null;

				$date = new JDate($mktime);
	...
	*/
	...
	/**
	* Reformate a list of date values.
	*
	* @access	public static
	* @param	array	$list	List of items.
	* @param	string	$key	Field name containing the date to format.
	...
	* @return	void
	*/
	public static function listFormat($list, $key, $format)
	{
