...
function modChrome_right($module, &$params, &$attribs)
{
	if ($module->content)
	{
		echo '<div class="right'.htmlspecialchars($params->get('moduleclass_sfx'), ENT_COMPAT, 'UTF-8').'">';
		if ($module->showtitle)
		{
...
function modChrome_footer($module, &$params, &$attribs)
{
	if ($module->content)
	{
		echo '<div class="'.htmlspecialchars($params->get('moduleclass_sfx'), ENT_COMPAT, 'UTF-8').'">';
		if ($module->showtitle)
		{
			echo "<h4>" . $module->title . "</h4>";
		}
